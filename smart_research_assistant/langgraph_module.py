...
def create_research_plan(state: ResearchState) -> ResearchState:
    ...
    if not research_plan:
        research_plan = [
            "Perform a general literature review on the query."
        ]  # ✅ FIX: prevent blocking if LLM returns empty
...
def execute_research(state: ResearchState) -> ResearchState:
    ...
    if research_plan is None or current_step is None:
        errors = new_state.get("errors") or []
        new_state["errors"] = errors + ["Research plan or current step is not set."]
        new_state["status"] = "error"
        return new_state

    step_description = research_plan[int(current_step)]

    SYSTEM_MESSAGE = SystemMessage(
        content="""
    Convert the research step into specific search queries that would help gather information related to this step. Provide 2-3 search queries."""
    )
...
def analyze_information(state: ResearchState) -> ResearchState:
    ...
    SYSTEM_MESSAGE = SystemMessage(
        content="""
        Analyze the following research information. Identify key insights, patterns, and potential conclusions. Be objective and thorough in your analysis.
        """
    )
...
def generate_summary(state: ResearchState) -> ResearchState:
    ...
    QUESTION_SYSTEM_MESSAGE = SystemMessage(
        content="""
        Based on the research analysis, suggest 4-5 follow-up questions that could help deepen the understanding of the topic or explore related areas."""
    )
...
def should_continue_research(state: ResearchState) -> str:
    ...
    if state.get("status") in ["summarizing", "completed"]:
        return "end"
...
class ResearchAssistantLanggraph(ResearchAssistantLangchain):
    ...
    def execute_research(self, query: str) -> ResearchState:
        ...
        result = self.graph.invoke(initial_state, {"recursion_limit": 200})  # type: ignore
